...
/mob/living/carbon/adjustToxLoss(amount, updating_health = TRUE, forced = FALSE, toxins_type = TOX_DEFAULT)
	...
	if(world.time < next_tox_vomit)
		return .
	if(new_tox >= 50)
		vomit(10, blood = TRUE, stun = TRUE, distance = 2, message = TRUE, vomit_type = VOMIT_TOXIC, harm = TRUE, force = TRUE)
		next_tox_vomit = world.time + (12 SECONDS)
	else if(new_tox >= 25)
		vomit(10, blood = FALSE, stun = TRUE, distance = 2, message = TRUE, vomit_type = VOMIT_TOXIC, harm = TRUE, force = TRUE)
		next_tox_vomit = world.time + (12 SECONDS)
	else if(new_tox >= 10)
